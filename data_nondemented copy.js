...
const nonDementedMMSEScores = [];
let sumMMSE = 0;
let countNonDemented = 0;
...
for (let i = 1; i < rows.length; i++) {
    ...
    if (group === 'Nondemented' && !isNaN(mmseScore)) {
        nonDementedMMSEScores.push(mmseScore);
        sumMMSE += mmseScore;
        countNonDemented++;
    }
}

const averageMMSE = sumMMSE / countNonDemented;

console.log('Non-demented MMSE Scores:', nonDementedMMSEScores);
